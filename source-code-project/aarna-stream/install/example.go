...
func setupSCTP(ip *string) (conn *sctp.SCTPConn, info *sctp.SndRcvInfo) {
...
func initRAN(ip *string) (t *testSession) {

	t = new(testSession)
	gnb := ngap.NewNGAP("example.json")
	gnb.SetDebugLevel(1)

	conn, info := setupSCTP(ip)
...
func (t *testSession) setupN3Tunnel(ctx context.Context, tunnel *string) {
...
func main() {
	var testName = flag.String("test", "initRAN", "test to execute")
	var ip = flag.String("ip", "localhost", "destinaion ip address")
	var tunnelName = flag.String("gtp", "gtp0", "gtp_tunnel name")

	if len(os.Args) < 5 {
		fmt.Printf("Usage: example -ip <AMF SCTP IP > -test registerUE \n")
		fmt.Printf("Usage: example -ip <AMF SCTP IP > -test setupPDUSession -gtp <gtp tunnel name>\n")
		return
	}
	...
	flag.Parse()
	fmt.Printf("testname %s IP %s\n", *testName, *ip)

	if *testName == "registerUE" {
		t := initRAN(ip)
		t.initUE()
		t.registrateUE()
	...
	if *testName == "setupPDUSession" {
		...
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		t.setupN3Tunnel(ctx, tunnelName)
		time.Sleep(time.Second * 1)
