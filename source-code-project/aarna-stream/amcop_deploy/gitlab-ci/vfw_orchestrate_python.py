...
try:
    ...
    if res3.status_code != 200 and res3.status_code != 201:
        print("Onboard cluster command returned non 200 code\n")
        print("Onboard cluster command response code is: %s\n", res3.status_code)
        res3.raise_for_status()
        exit(1)
...
try:
    ...
    #url_emco_net = 'http://%s:%s/v2/ncm/%s/clusters/%s/provider-networks' % (amcop_deployment_ip, middle_end_port, provider_name, cluster_name)
    url_emco_net = 'http://%s:30480/v2/ncm/%s/clusters/%s/provider-networks' % (amcop_deployment_ip, provider_name, cluster_name)
    time.sleep(2)
...
try:
    ...
    #url_unprotected_net = 'http://%s:%s/v2/ncm/%s/clusters/%s/provider-networks' % (amcop_deployment_ip, middle_end_port, provider_name, cluster_name)
    url_unprotected_net = 'http://%s:30480/v2/ncm/%s/clusters/%s/provider-networks' % (amcop_deployment_ip, provider_name, cluster_name)
    time.sleep(2)
...
try:
    ...
    #url_protected_net = 'http://%s:%s/v2/ncm/%s/clusters/%s/networks' % (amcop_deployment_ip, middle_end_port, provider_name, cluster_name)
    url_protected_net = 'http://%s:30480/v2/ncm/%s/clusters/%s/networks' % (amcop_deployment_ip, provider_name, cluster_name)
    time.sleep(2)
...
try:
    ...
    cluster_name = "clu-48"
    #url_net_apply = 'http://%s:%s/v2/ncm/%s/clusters/%s/apply' % (amcop_deployment_ip, middle_end_port, provider_name, cluster_name)
    url_net_apply = 'http://%s:30480/v2/ncm/%s/clusters/%s/apply' % (amcop_deployment_ip, provider_name, cluster_name)
    time.sleep(2)
...
try:
    ...
    f = open("./tt.json", "rb")
    data = json.load(f)
    file1 = home_dir + "/sink.tgz"
    file2 = home_dir + "/packetgen.tgz"
    file3 = home_dir + "/firewall.tgz"
    file4 = home_dir + "/profile.tar.gz"
    files = {
...
try:
    ...
    url6 = 'http://%s:30480/middleend/projects/%s/composite-apps?filter=depthAll' % (amcop_deployment_ip, project_name)
    time.sleep(1)
...
print("\n")
print("--------------------------------------------------------------\n")
print("vFW orchestration is successful and starting vFW undeploy process")
print("--------------------------------------------------------------\n")
print("\n")
time.sleep(5)
...
try:
    print("---------------Deleting vFW DIG---------------")
    time.sleep(5)
    url14 = 'http://%s:%s/middleend/projects/%s/composite-apps/%s/v1/deployment-intent-groups/%s?operation=deleteAll' % (
        amcop_deployment_ip, middle_end_port, project_name, comp_appname, dig_name)
    time.sleep(1)
    res14 = requests.delete(url14)
    print("Status code is : %s", res14.status_code)
...
try:
    print("---------------Deleting vFW service---------------")
    url5 = 'http://%s:%s/middleend/projects/%s/composite-apps/%s/v1' % (
        amcop_deployment_ip, middle_end_port, project_name, comp_appname)
    time.sleep(1)
    res15 = requests.delete(url5)
...
try:
    ...
    url6 = 'http://%s:30480/v2/dcm/projects/%s/logical-clouds/lc2/terminate' % (amcop_deployment_ip, project_name)
    res16 = requests.post(url6)
    ...
    url7 = 'http://%s:30480/v2/dcm/projects/%s/logical-clouds/lc2/cluster-references/lc2-provider-1-clu-48' % (amcop_deployment_ip, project_name)
    res17 = requests.delete(url7)
    ...
    url8 = 'http://%s:30480/v2/dcm/projects/%s/logical-clouds/lc2' % (amcop_deployment_ip, project_name)
    res18 = requests.delete(url8)
...
# python vfw_orchestrate_python.py 192.168.122.155 30480 30415 30461 30477
